...
async def obtener_resumen_metricas(
    # El usuario deberá pasar la fecha en el formato 'YYYYMM' (ej: 202510)
    # 🚨 CORRECCIÓN: Usamos r"..." (Raw String) para evitar el SyntaxWarning de \d
    fecha: str = Query(..., 
                       regex=r"^\d{6}$", 
                       description="Fecha en formato YYYYMM (Ej: 202510)"), 
    # Aseguramos el acceso con la dependencia que creaste
    usuario: Dict[str, Any] = Depends(validar_acceso_cotizaciones) 
) -> JSONResponse:
    """
    Ejecuta el query SQL para obtener el resumen de cotizaciones (Abiertas, Cerradas, Vendidas, etc.)
    para un mes y año específicos.
    """
    
    # Adaptación: Tomamos los últimos 4 dígitos (YYMM) para el query
    parametro_sql_fecha = fecha[-4:]
    
    SQL_QUERY = f"""
-- COTIZACIONES ABIERTAS, CERRADAS, VENDIDAS, TOTAL GENERAL,ENTREGADAS A TIEMPO, ENTRAGADA CON RETRASO, EN RIESGO
SELECT 
    CASE 
    ...
    -- 🔹 total cotizaciones (no cuenta vendidas ni entregadas ni en riesgo en total general)
    CASE 
        WHEN GROUPING(Clasificacion) = 1 THEN 
            SUM(CASE 
                    WHEN Clasificacion NOT IN ('Vendida', 'Entregada a Tiempo', 'Entregada con Retraso', 'En Riesgo') 
                    THEN Total_Cotizaciones 
                    ELSE 0 
                END)
        ELSE SUM(Total_Cotizaciones)
    END AS Total_Cotizaciones,

    -- 🔹 total kilos (no cuenta entregadas ni en riesgo en total general)
    CASE 
        WHEN GROUPING(Clasificacion) = 1 THEN 
            FORMAT(SUM(CASE 
    ...
    -- 🔹 total monto (solo en vendidas y cerradas)
    CASE 
    ...
    -- 🔹 promedio price per kg (solo en vendidas, incluye finalizadas)
    CASE 
        WHEN GROUPING(Clasificacion) = 1 THEN NULL
...
FROM (
    ...
    LEFT JOIN status st 
        ON q.status = st.idStatus
    LEFT JOIN quotation_systems qs 
        ON q.idQuotation = qs.quotationId
    ...
    -- 🔹 Cerradas (finalizadas + vendidas)
    SELECT 
        'Cerrada' AS Clasificacion,
        COUNT(DISTINCT q.idQuotation) AS Total_Cotizaciones,
        SUM(qs.totalKgSold) AS Total_Kilos,
    ...
    -- 🔹 Entregadas a Tiempo
    SELECT 
        'Entregada a Tiempo' AS Clasificacion,
        COUNT(DISTINCT q.idQuotation) AS Total_Cotizaciones,
        SUM(qs.totalKgSold) AS Total_Kilos,
        0 AS Total_Monto,
        NULL AS AVG_PricePerKg
    ...
    SELECT 
        'Entregada con Retraso' AS Clasificacion,
        COUNT(DISTINCT q.idQuotation) AS Total_Cotizaciones,
        SUM(qs.totalKgSold) AS Total_Kilos,
        0 AS Total_Monto,
        NULL AS AVG_PricePerKg
    ...
    SELECT 
        ...
        (
            ...
            FROM quotation q2
            LEFT JOIN sale_status s2 
                ON q2.saleStatus = s2.idSalestatus
            LEFT JOIN status st2 
                ON q2.status = st2.idStatus
            LEFT JOIN quotation_systems qs2 
                ON q2.idQuotation = qs2.quotationId
            WHERE q2.quotationDate = {parametro_sql_fecha}
              AND (
                   s2.Nombe = 'Vendido'
                   OR st2.status IN ('Finalizada', 'Finalizada con retraso', 'Finalizada con retraso de ventas')
              )
              AND qs2.pricePerKg IS NOT NULL
              AND qs2.pricePerKg <> 0
        ) AS AVG_PricePerKg
    FROM quotation q
    LEFT JOIN sale_status s 
        ON q.saleStatus = s.idSalestatus
...
ORDER BY 
    ...
    try:
        ...
        # Convertimos Decimals a float para JSON
